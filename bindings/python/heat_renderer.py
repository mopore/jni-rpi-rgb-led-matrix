...
class HeatColorizer:
    def __init__(self, collector: JsonDataCollector) -> None:
        ...
        if json_string is not None:
            sensor_data: list[list[float]] = json.loads(
...
class HeatvisionRenderer(Renderer):
    ...
    def __init__(self, mqtt_bridge: MqttBridge) -> None:
        def handle_message(topic: str, text: str) -> None:
            self.receive_heatvision_data(text)

        mqtt_bridge.subscribe_with_cb(TOPIC_DATA_NAME, handle_message)
